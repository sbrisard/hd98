...
//  }
//
//  double delta_eps[n * sym], eps1[n * sym], sig2_act[n * sym];
//  double sig2_exp[n * sym];
//  for (size_t i = 0; i < n * sym; i++) {
//    delta_eps[i] = 0.;
//    eps1[i] = 0.;
//    sig2_act[i] = 0.;
//    sig2_exp[i] = 0.;
...
//  std::vector<double> C2_act(n * sym * sym);
//  std::vector<double> C2_exp(n * sym * sym);
//
//  for (size_t i = 0; i < n; i++) {
//    eps1[sym * i] = 1.e-4 * i;
//  }
...
//  }
//
//  hd98_global_update(n, phase, mat.data(), delta_eps, eps1, omega1.data(),
//                     sig2_act, omega2_act.data(), C2_act.data());
//  double *omega1_i = omega1.data();
//  double *omega2_i = omega2_exp.data();
//  for (size_t i = 0; i < n; i++) {
//    HD98_Material const *mat_i = mat[phase[i]];
//    mat_i->type->update(mat_i, delta_eps + sym * i, eps1 + sym * i,
//                        omega1_i, sig2_exp + sym * i, omega2_i,
//                        C2_exp.data() + sym * sym * i);
//    omega1_i += mat_i->type->niv;
//    omega2_i += mat_i->type->niv;
//  }
//
//  assert_array_equal(n * sym, sig2_act, sig2_exp, 1e-15, 1e-15);
//  assert_array_equal(m, omega2_act.data(), omega2_exp.data(), 1e-15, 1e-15);
//  assert_array_equal(n * sym * sym, C2_act.data(), C2_exp.data(),
//                     1e-15, 1e-15);
//  printf(" OK\n");
//}
